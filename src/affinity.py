...
DELTA_MIN = -2
DELTA_MAX = 2
PROTECTED_DELTA_MIN = -0.5
PROTECTED_AFFINITY_MIN = 60
    ...
    def apply_delta(self, delta_map: dict):
        ...
        Args:
            delta_map: {qq: delta}，delta 会 clip 到 [-2, +2]；
                       protected_qq 负向 delta 限制 -0.5，最低 60
        """
        for qq, delta in delta_map.items():
            is_protected = (qq == self.protected_qq)
            if is_protected:
                delta = max(PROTECTED_DELTA_MIN, min(DELTA_MAX, delta))
            else:
                delta = max(DELTA_MIN, min(DELTA_MAX, delta))
            old = self.get(qq)
            new = old + delta
            if is_protected:
                new = max(PROTECTED_AFFINITY_MIN, min(AFFINITY_MAX, new))
            else:
            ...
            logger.debug(f"亲密度 {qq}: {old} -> {new} (delta={delta}{'，受保护' if is_protected else ''})")
        self._save()
